          ...
          <?php } // end title only style
            ...
            if ( has_post_thumbnail() ) { 
              if( $style === 'list_featured_first_row_tall' && $r_post_count <= $columns ) {
                 echo'<a href="' . esc_url(get_permalink()) . '" class="'.esc_attr($list_featured_image_class).'"><span class="post-featured-img" style="background-image: url('.get_the_post_thumbnail_url($post->ID, 'regular', array('title' => '')).');"></span></a>'; 
              } else {
